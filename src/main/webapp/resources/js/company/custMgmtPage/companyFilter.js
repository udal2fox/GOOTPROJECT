...
function filter() {
	// 체크박스 '전체'값 이벤트 리스너 등록 
	document.getElementById('comArea_typeAll').addEventListener('change', function(event) {
...
document.querySelectorAll('.searchbar_checkbox_filter').forEach(function(checkbox) {
    checkbox.addEventListener('change', function() {
        ...
        document.getElementById('comArea_typeAll').checked = isTypeAllChecked;
...
function filterComArea() {
    ...
    // '전체' 체크 상태 확인
    let isAllTypeChecked = document.getElementById('comArea_typeAll').checked;
...
// 각 기업을 순회하면서 필터링 적용
document.querySelectorAll('.companyList').forEach(function(companyList) {
    let type = companyList.getAttribute('data-type');
   
    // 지역이 필터에 포함되는 경우 보여주기, 그렇지 않은 경우 숨기기
...
function goToPage(page) {
    ...
    let filteredConsults = getFilteredCompany();
...
{
    let totalPages = Math.ceil(getFilteredCompany().length / amount);
//    if (getFilteredCompany().length % amount === 0) 
//    {
//        totalPages--;
//    }
...
{
    let typeFilters = Array.from(document.querySelectorAll('.searchbar_checkbox_filter[data-filter="comArea_type"]')).map(function(checkbox) {
    return checkbox.value;
});
